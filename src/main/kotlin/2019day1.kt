...
fun part1(): Long {
//    println("12: ${calcFuel(12)}")
//    println("14: ${calcFuel(14)}")
//    println("1969: ${calcFuel(1969)}")
//    println("100756: ${calcFuel(100756)}")

    val input = "src/main/kotlin/2019day1.input.txt".let { that -> File(that).readLines().map { calcFuel(it.toInt()) } }
    val part1 = input.fold(0) { acc:Long, num -> acc + num}
    return part1
}

fun calcFuel(mass: Int): Long {
    val temp = Math.round((mass / 3).toDouble())  - 2
    return temp
}
...
fun part2(): Long {
    val input = "src/main/kotlin/2019day1.sample.input.txt".let { that -> File(that).readLines().map { calcFuel2(it.toInt()) } }
    val part2 = input.fold(0) { acc:Long, num -> acc + num}

    return part2
}

fun calcFuel2(mass: Int): Long {
    var loop = true
    var currentValue = mass.toLong()
    do {
        val temp = calcFuel(currentValue.toInt())
        if (temp > 0) currentValue = temp
